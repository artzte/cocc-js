...
- **JS fundamentals first, browser as the exercise vehicle.** The COCC catalog description is browser-centric, but the course teaches the language deeply using _You Don't Know JS Yet_ as the primary text. Students learn DOM/HTML5 APIs self-directed via MDN; lectures focus on JS itself.
...
- **Cherry-pick _Scope & Closures_.** Course outcomes override book chapter sequence. Appendix material (implied scopes, TDZ edge cases, named function nuances) is optional self-study.
