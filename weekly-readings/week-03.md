...
**Source**: _You Don't Know JS Yet: Get Started_ — Chapter 2: Surveying JS (Functions and Comparisons)
...
- **Function Declarations**:
  ...
  - The identifier is associated with the function at **compile time** (facilitating _function hoisting_).
...
- It uses **identity/reference equality**: two variables are equal only if they reference the _exact same object in memory_.
...
- **Behavior**:
  ...
  - _Tip_: Avoid using `==` with empty strings (`""`), `0`, or `false` to avoid confusing corner cases.
