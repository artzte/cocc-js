...
**Source**: _You Don't Know JS Yet: Get Started_ — Chapter 2: Surveying JS (Values through Declaring and Using Variables)
...
- **`const`**:
  ...
  - **Mutation Warning**: Storing objects or arrays in a `const` prevents re-assigning the variable itself, but the _contents_ of the object or array can still be mutated (e.g., `const arr = []; arr.push(1);` is valid). It is best to use `const` only with primitive values to avoid mutation confusion.
- **Other Declarations**:
