...
* **Screencast Capture Zone**: Lower-left quadrant
  ...
  * **Features**:
    * Camera viewfinder corner brackets and `(0, 0)`, `(1920, 0)`, `(0, 1080)`, `(1920, 1080)` corner labels.
...
* Pillow (`PIL`) & NumPy:
  ...
  ```
* Ubuntu font package (default on Ubuntu):
  `/usr/share/fonts/truetype/ubuntu/Ubuntu-B.ttf`, `Ubuntu-M.ttf`, `Ubuntu-R.ttf`, `UbuntuMono-B.ttf`, `UbuntuMono-R.ttf`
...
The script also automatically places fresh copies into `~/Pictures/` and `~/.local/share/backgrounds/`.
...
* **Dimensions & Placement**: Adjust `GUIDE_W`, `GUIDE_H`, `GUIDE_X`, and `GUIDE_Y` at the top of the file.
